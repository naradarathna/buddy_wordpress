...
<html lang="en-US">
   <head>
      ...
      <title>BUDDY | Guided Meditation</title>
   ...
   <body onload="onLoad();">
      ...
      <div class="container-fluid">
         ...
         <div class="container-fluid" style="margin-top:10px">
            <div class="row" style="color:black">
               <!--<hr>-->
               <h1 style="width: 100%; text-align: center;font-size:50px;background-color:black;color:white;padding-bottom:5px;">Guided Meditation</h1>
               <!--<hr>-->
      ...
      <script>
         ...
         function loadAllVideos(){
             // All Video
         	var videos = "https://buddytorest.co.uk/php_request?func=meditation";
             ...
             videosRequestXhttp.onload = function() {
             	if (this.readyState == 4 && this.status == 200) {
                     ...
                     if(responseArr)
                     {
                         var fav_videos = "https://buddytorest.co.uk/php_request?func=favmeditation&user_id=" + getCookie("user_buddy");
                         	...
                         	if (this.readyState == 4 && this.status == 200) {
                                 ...
                                 if(favresponseArr.length > 0)
                                 {
                                     ...
                                     for(var i =0 ; i < responseArr.length; i++)
                                     {
                                         ...
                                         {
                                             if(responseArr[i].meditation_id == favresponseArr[j].meditation_id)
                                             {
                                                 if(!fav_video.includes(responseArr[i].meditation_id))
                                                 {
                                                     fav_video.push(responseArr[i].meditation_id);
                                                 }
                                             }
                                         }
                                     }
                                     ...
                                     for(var i =0 ; i < responseArr.length; i++)
                                     {
                                         if(fav_video.includes(responseArr[i].meditation_id))
                                         {
                                             console.log("Include - " + responseArr[i].meditation_id);
                                             fav_html_content = fav_html_content + 
                                                 "<div class='col-sm-2' id='Yoga_22' style='margin-bottom:25px;margin-right:25px'>" + 
                                                 "<div style='padding:56.25% 0 0 0;position:relative;'>" + 
                                                 "<iframe src='" + responseArr[i].meditation_url + "' style='position:absolute;top:0;left:0;width:100%;height:100%;' frameborder='0' allow='fullscreen; picture-in-picture' allowfullscreen></iframe>" + 
                                                 "</div>" +
                                                 "<span style='font-size:20px;inline-size: 80%;overflow: hidden;white-space: nowrap;display: inline-block;text-overflow: ellipsis;'>"+ responseArr[i].meditation_title +"</span>" +
                                                 "<i id='next_prev' class='bi bi-heart-fill' style='color:black;font-size:25px;float: right;padding-top:5px;padding-right:5px;' onClick='unsetFavourite("+ responseArr[i].meditation_id +")'></i>" + 
                                                 "</div>";
                                         }
                                         else
                                         {
                                             html_content = html_content + 
                                                 ...
                                                 "</div>" +
                                                 "<span style='font-size:20px;inline-size: 80%;overflow: hidden;white-space: nowrap;display: inline-block;text-overflow: ellipsis;'>"+ responseArr[i].meditation_title +"</span>" +
                                                 "<i id='next_prev' class='bi bi-heart' style='color:black;font-size:25px;float: right;padding-top:5px;padding-right:5px;' onClick='setFavourite("+ responseArr[i].meditation_id +")'></i>" + 
                                                 "</div>";
                                         }
                                     }
                                     ...
                                     if(html_content.length > 0){
                                        calender_title.innerHTML = html_content;
                                         ...
                                         calender_title.innerHTML = "<h2>All guided meditation videos are favourite</h2>";;
                                     }
                                 ...
                                 else
                                 {
                                     ...
                                     for(var i =0 ; i < responseArr.length; i++)
                                     {
                                         //console.log(responseArr[i].meditation_url);
                                         html_content = html_content + 
                                             "<div class='col-sm-2' id='Yoga_22' style='margin-bottom:25px;margin-right:25px'>" + 
                                             "<div style='padding:56.25% 0 0 0;position:relative;'>" + 
                                             "<iframe src='" + responseArr[i].meditation_url + "' style='position:absolute;top:0;left:0;width:100%;height:100%;' frameborder='0' allow='fullscreen; picture-in-picture' allowfullscreen></iframe>" + 
                                     ...
                                     calender_title.innerHTML = html_content;
                                     var fav_div = document.getElementById("favorites");
                                     fav_div.innerHTML = "<h2>No guided meditation videos</h2>";
                                 }
                         	}
                         };
         ...
         function setFavourite(video_id){
             // Set favourite Video
         	var videos = "https://buddytorest.co.uk/php_request?func=meditationSetFav&user_id=" + getCookie("user_buddy") + "&meditation_id=" + video_id;
             const videosRequestXhttp = new XMLHttpRequest();
             videosRequestXhttp.onload = function() {
             	if (this.readyState == 4 && this.status == 200) {
             		var responseArr = JSON.parse(this.responseText);
         ...
         function unsetFavourite(video_id){
             // Set favourite Video
         	var videos = "https://buddytorest.co.uk/php_request?func=meditationUnsetFav&user_id=" + getCookie("user_buddy") + "&meditation_id=" + video_id;
             const videosRequestXhttp = new XMLHttpRequest();
             videosRequestXhttp.onload = function() {
             	if (this.readyState == 4 && this.status == 200) {
             		var responseArr = JSON.parse(this.responseText);
