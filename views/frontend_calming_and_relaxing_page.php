...
<html lang="en-US">
   <head>
      ...
      <title>BUDDY | Calming and Relaxing</title>
   ...
   <body onload="onLoad();">
      ...
      <div class="container-fluid">
         ...
         <div class="container-fluid" style="margin-top:10px">
            <div class="row" style="color:black">
               <hr>
               <h1 style="width: 100%; text-align: center;font-size:50px;background-color:black;color:white;padding-bottom:5px;">Calming and Relaxing</h1>
               <hr>
               <h2 style="font-size:25px;margin-bottom:20px;">Favorites</h3>
            </div>
            ...
            <div class="row" style="margin-top:25px; color:black">
               ...
               <h2 style="font-size:25px;">All</h3>
            </div>
      ...
      <script>
                         	...
                         	if (this.readyState == 4 && this.status == 200) {
                                 ...
                                 if(favresponseArr.length > 0)
                                 {
                                     ...
                                     for(var i =0 ; i < responseArr.length; i++)
                                     {
                                         if(fav_video.includes(responseArr[i].relaxing_id))
                                         {
                                             console.log("Include - " + responseArr[i].relaxing_id);
                                             fav_html_content = fav_html_content + 
                                                 "<div class='col-sm-2' id='relaxing_22' style='margin-bottom:25px;margin-right:25px'>" + 
                                                 '<audio controls style="width: 100%;" id="audio_with_controls">' + 
                                                 '<source src="' + responseArr[i].relaxing_url +'" type="audio/mpeg">' +
                                                 'Your browser does not support the audio element.' +
                                                 '</audio>' +
                                                 "<span style='font-size:20px;inline-size: 80%;overflow: hidden;white-space: nowrap;display: inline-block;text-overflow: ellipsis;'>"+ responseArr[i].relaxing_title +"</span>" +
                                                 "<i id='next_prev' class='bi bi-heart-fill' style='color:black;font-size:25px;float: right;padding-top:5px;padding-right:5px;' onClick='unsetFavourite("+ responseArr[i].relaxing_id +")'></i>" + 
                                                 "</div>";
                                         }
                                         else
                                         {
                                             html_content = html_content + 
                                                 ...
                                                 '</audio>' +
                                                 "<span style='font-size:20px;inline-size: 80%;overflow: hidden;white-space: nowrap;display: inline-block;text-overflow: ellipsis;'>"+ responseArr[i].relaxing_title +"</span>" +
                                                 "<i id='next_prev' class='bi bi-heart' style='color:black;font-size:25px;float: right;padding-top:5px;padding-right:5px;' onClick='setFavourite("+ responseArr[i].relaxing_id +")'></i>" + 
                                                 "</div>";
                                         }
                                     }
                                     ...
                                     if(html_content.length > 0){
                                        calender_title.innerHTML = html_content;
                                         ...
                                         calender_title.innerHTML = "<h2>All calming and relaxing musics are favourite</h2>";;
                                     }
                                 ...
                                 else
                                 {
                                     ...
                                     for(var i =0 ; i < responseArr.length; i++)
                                     {
                                         //console.log(responseArr[i].relaxing_url);
                                         html_content = html_content + 
                                             "<div class='col-sm-2' id='relaxing_22' style='margin-bottom:25px;margin-right:25px'>" + 
                                             '<audio controls style="width: 100%;" id="audio_with_controls">' + 
                                             '<source src="' + responseArr[i].relaxing_url +'" type="audio/mpeg">' +
                                     ...
                                     calender_title.innerHTML = html_content;
                                     var fav_div = document.getElementById("favorites");
                                     fav_div.innerHTML = "<h2>No favourite calming and relaxing musics</h2>";
                                 }
                         	}
                         };
         ...
         function setFavourite(video_id){
             // Set favourite Video
         	var videos = "https://buddytorest.co.uk/php_request?func=relaxingSetFav&user_id=" + getCookie("user_buddy") + "&relaxing_id=" + video_id;
             const videosRequestXhttp = new XMLHttpRequest();
             videosRequestXhttp.onload = function() {
             	if (this.readyState == 4 && this.status == 200) {
             		var responseArr = JSON.parse(this.responseText);
         ...
         function unsetFavourite(video_id){
             // Set favourite Video
         	var videos = "https://buddytorest.co.uk/php_request?func=relaxingUnsetFav&user_id=" + getCookie("user_buddy") + "&relaxing_id=" + video_id;
             const videosRequestXhttp = new XMLHttpRequest();
             videosRequestXhttp.onload = function() {
             	if (this.readyState == 4 && this.status == 200) {
             		var responseArr = JSON.parse(this.responseText);
